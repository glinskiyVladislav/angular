...
@Pipe
@Реализация поиска
@Изменение валюты   +  там же работа с select (выпадающий список)
@Добавленние в корзину
@Оформление заказа
@Deploy
@Модули :
  @Флеш сообщения
...
Некоторые методы что не реализованы
...
---------------------------------------- @Изменение валюты ----------------------------------
@Изменение валюты
@вставляем компоненту
@Создать сервис
@Интерфейс Currency
@Разметка для фильтра курса
@Реагирование на изменение в html
---------------------------------------------------------------------------------------------
-----------------------------------------@Добавленние в корзину------------------------------
@Создать  компоненту корзины
@Создать компоненту клиентской части
@Создать сервис корзны
 
@Оформление заказа
@Создание сервиса для работы с заказом - NOTE в бд нужно создать коллекцию
...
------------------------------------------ @Deploy ------------------------------------------
Deploy на firebase:
  @Скопировать environment
  Шаг 2:
    Зайти на firebase -> hosting
    и выполнить команду npm install -g firebase-tools
    нажать продолжить и сследовать инструкциям
    выбрать firestore и hosting в firebase init
    firestore rules - пустой
    indexes - пустой
    папка - dist
    потом y - как сингл пэйдж
    после удалить папку dist
    и в терминале запустить команду
    ng build --prod
    После билда запустить команду
    firebase deploy
    после чего будет адрес с приставкой firebase
    https://books-store-lesson-1d68c.firebaseapp.com

Deploy c помощью file zila    
в терминале запустить команду
    ng build --prod --base-href="Адрес подпапки на сервере" 10 урок деплой 12 минут  
...
---------------------------------------------------------------------------------------------
--------------------------------------- @Кастомные дерективы --------------------------------
Дерективы это набор инструкций которые применяются при взаимодейстии с html и добавляют 
некое поведение или дополнительное свойство элементу либо создают новые элементы DOM дерева
Типы:
  - Атрибутные дерективы - добавляют поведение или меняют текущее поведение или меняют стили элемента - Не меняют разметку
    например ngStyle
  - Структурные - меняют разметку и требуют шаблон 
    например ngIf ngFor
Разновидности 
  Встроенные
  Пользовательские - которые создаем сами и переиспользовать если нужно
  
@Команда создания
ng g directive directives/custom-color --spec=false
Атрибутные дерективы:
  @Структура файла дерективы
  @Пример в html
...
Структурные дерективы
 ...
 @Пример в html структурной дерективы
